(function(){
  ...
  async function restoreInterviewUI(sid){
    ...
    try{
      ...
      if(r.code===200){
        ...
        // Can't restore interview — go to report or start fresh
        toast('无法恢复面试状态，请尝试查看报告','error');
        showJDInput();
      }
    }catch(e){
      toast('恢复面试失败','error');
  ...
  function buildInterviewUI(){
    ...
    c.innerHTML='<div class="iv-card">'+
      ...
      '<div class="iv-input-area">'+
        '<textarea id="input-interview-answer" placeholder="输入你的回答后点击发送..." rows="2"></textarea>'+
      ...
      '<div style="margin-top:12px;display:flex;justify-content:space-between;align-items:center">'+
        '<span style="font-size:.75rem;color:var(--text-muted)">Enter 发送 · Shift+Enter 换行</span>'+
        '<button class="btn-sm" id="btn-end-interview" onclick="window._endInterview()" style="color:var(--danger);border-color:var(--danger)">&#x23F9; 结束面试</button>'+
      '</div></div>';
  }
  ...
  window._endInterview=async function(){
    ...
    var btn=$('btn-end-interview');if(btn){btn.disabled=true;btn.textContent='正在生成报告...'}
    try{
      ...
      if(r.code===200){
        ...
        if(btn){btn.disabled=false;btn.textContent='结束面试并生成报告'}
      }
    }catch(e){toastErr(e);if(btn){btn.disabled=false;btn.textContent='结束面试并生成报告'}}
  };
  ...
  async function loadInterviewHistory(){
    ...
    try{
      ...
      if(r.code===200&&r.data){
        var items=r.data.filter(function(s){return s.status!=='created'||s.overall_score>0}).slice(0,15);
  ...
  window._viewPastResult=async function(sid){
    ...
    try{
      ...
      if(r.code===200&&r.data){
        var d=r.data; var score=d.overall_score||0;
        var cls=score>=7?'score-high':score>=4?'score-mid':'score-low';
        var dims=''; if(d.dimension_score){for(var k in d.dimension_score){
          var dv=d.dimension_score[k]; var dc=dv>=7?'high':dv>=4?'mid':'low';
          dims+='<div class="dim-bar"><div class="dim-label"><span>'+k+'</span><span>'+dv.toFixed(1)+'</span></div><div class="dim-track"><div class="dim-fill '+dc+'" style="width:'+(dv*10)+'%"></div></div></div>';
        }}
        var weak=(d.weak_areas||[]).map(function(w){return '<span class="tag" style="border-color:var(--danger);color:var(--danger)">'+w+'</span>'}).join(' ');
        c.innerHTML='<div class="iv-card"><h3>&#x1F4CA; 历史面试报告</h3>'+
          '<div style="text-align:center;margin:16px 0"><span class="score-badge '+cls+'">总分 '+score.toFixed(1)+'</span></div>'+
          (dims?'<div class="iv-section"><h4>维度评分</h4>'+dims+'</div>':'')+
          (weak?'<div class="iv-section"><h4>薄弱项</h4><div class="tag-list">'+weak+'</div></div>':'')+
          (d.summary?'<p style="color:var(--text-muted);margin-top:12px">'+d.summary+'</p>':'')+
          '<button class="btn btn-secondary" onclick="window.showJDInput()" style="margin-top:12px">开始新面试</button></div>';
        setProgress('report');
      } else {
  ...
  // History modal
  $('btn-interview-history').addEventListener('click',async function(){
    ...
    try{
      var r=await api('GET','/sessions?user_id='+encodeURIComponent(getUserId()));
      if(r.code===200&&r.data){
        var items=r.data.filter(function(s){return s.overall_score>0});
  ...
  async function loadDocs(){
    ...
    try{
      ...
      $('knowledge-content').innerHTML='<div class="upload-zone" id="upload-zone"><p>拖拽文件到此处上传，或点击选择</p><input type="file" id="file-input" multiple hidden></div><h3 style="margin:12px 0">已上传文档</h3><div id="doc-list"></div>';
      if(d.code===200&&d.data){
        var list=$('doc-list');
        d.data.forEach(function(doc){list.innerHTML+='<div class="doc-item"><span>'+doc.source_file+'</span><button class="btn-sm" onclick="window._deleteDoc(\''+doc.id+'\')">删除</button></div>'});
      }
      var zone=$('upload-zone');zone.addEventListener('click',function(){$('file-input').click()});
      $('file-input').addEventListener('change',function(){if(this.files.length>0) uploadFiles(this.files)});
      ...
      zone.addEventListener('drop',function(e){e.preventDefault();uploadFiles(e.dataTransfer.files)});
    }catch(e){}
  }
